    ...
    @auth
        @if (request()->routeIs('admin.*'))
            <div x-show="mobile_menu_open && !desktop" x-transition:enter="transition-all duration-300 ease-out"
                ...
                class="fixed inset-0 z-40 glass-card lg:hidden">
            </div>

            <div class="flex h-screen" x-data="dashboardData()">
                <!-- Sidebar -->
                <livewire:partials.admin.sidebar :active="$page_slug" />
                ...
                <div class="flex-1 flex flex-col custom-scrollbar overflow-y-auto">
                    ...
                    <livewire:partials.admin.header :breadcrumb="$breadcrumb" />
                        ...
                        @if (request()->routeIs('user.*'))
                            <div class="container mx-auto px-1 xs:px-4 pt-8">
                                <div class="grid grid-cols-2 gap-2 sm:flex sm:gap-4 mb-8">
                                    ...
                                    <!-- Payments -->
                                    <a href="{{ route('user.payments') }}" wire:navigate
                                        class="flex-1 flex items-center justify-center gap-2 lg:text-xl xs:text-base text-sm py-3 px-3 xs:px-6 rounded-lg font-medium transition-colors duration-200 hover:opacity-90 

    ...
    <section>
        ...
        <div class="flex items-center justify-between glass-card rounded-2xl p-6 mb-6">
            ...
            <div class="flex gap-2">
                @if ($detailsOrder->booking_status == \App\Models\Booking::BOOKING_STATUS_PENDING)
                    <a href="{{ route('admin.om.index') }}"
                        class="inline-flex items-center gap-2 px-4 py-2 text-black rounded-lg transition-colors">
        ...
        <div class="glass-card rounded-2xl overflow-hidden">
            <div class="border-b border-zinc-700">
                <nav class="flex overflow-x-auto px-2 sm:px-6 space-x-4 sm:space-x-6 scrollbar-hide" aria-label="Tabs">
                    <button @click="activeTab = 'overview'"
                        :class="activeTab === 'overview' ? 'border-emerald-500 text-emerald-400' :
                            'border-transparent text-zinc-400 hover:text-zinc-300'"
                        class="shrink-0 py-3 px-2 sm:px-3 border-b-2 font-medium text-sm sm:text-base transition-colors">
                        Overview
                    </button>

                    <button @click="activeTab = 'billing'"
                        :class="activeTab === 'billing' ? 'border-emerald-500 text-emerald-400' :
                            'border-transparent text-zinc-400 hover:text-zinc-300'"
                        class="shrink-0 py-3 px-2 sm:px-3 border-b-2 font-medium text-sm sm:text-base transition-colors">
                        Billing Information
                    </button>
                    ...
                    <button @click="activeTab = 'addresses'"
                        :class="activeTab === 'addresses' ? 'border-emerald-500 text-emerald-400' :
                            'border-transparent text-zinc-400 hover:text-zinc-300'"
                        class="shrink-0 py-3 px-2 sm:px-3 border-b-2 font-medium text-sm sm:text-base transition-colors">
                        Addresses
                    </button>
                    ...
                    <button @click="activeTab = 'documents'"
                        :class="activeTab === 'documents' ? 'border-emerald-500 text-emerald-400' :
                            'border-transparent text-zinc-400 hover:text-zinc-300'"
                        class="shrink-0 py-3 px-2 sm:px-3 border-b-2 font-medium text-sm sm:text-base transition-colors">
                        Documents
                    </button>
                    ...
                    <button @click="activeTab = 'vehicle'"
                        :class="activeTab === 'vehicle' ? 'border-emerald-500 text-emerald-400' :
                            'border-transparent text-zinc-400 hover:text-zinc-300'"
                        class="shrink-0 py-3 px-2 sm:px-3 border-b-2 font-medium text-sm sm:text-base transition-colors">
                        Vehicle Details
                    </button>
                    ...
                    <button @click="activeTab = 'timeline'"
                        :class="activeTab === 'timeline' ? 'border-emerald-500 text-emerald-400' :
                            'border-transparent text-zinc-400 hover:text-zinc-300'"
                        class="shrink-0 py-3 px-2 sm:px-3 border-b-2 font-medium text-sm sm:text-base transition-colors">
                        Timeline
                    </button>
            ...
            <div x-show="activeTab === 'overview'" x-transition:enter="transition ease-out duration-200"
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div class="bg-zinc-800 rounded-lg p-4 border border-zinc-700">
                        ...
                        <span
                            class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium badge badge-soft {{ $detailsOrder->booking_status_color }}">
                    ...
                    <div class="bg-zinc-800 rounded-lg p-4 border border-zinc-700">
                        ...
                        <p class="text-zinc-100 font-medium capitalize">
                            {{ $detailsOrder->relation?->rental_range ?? 'N/A' }}</p>
                    </div>

                    <div class="bg-zinc-800 rounded-lg p-4 border border-zinc-700">
                        <p class="text-xs text-zinc-400 uppercase tracking-wider mb-1">Rental Duration</p>
                        <p class="text-zinc-100 font-medium">{{ $detailsOrder->rental_duration_days ?? 0 }} Days
                        </p>
                    </div>
                    <div class="bg-zinc-800 rounded-lg p-4 border border-zinc-700">
                    ...
                    <div class="bg-zinc-800 rounded-lg p-4 border border-zinc-700">
                        ...
                        <p class="text-zinc-100 font-medium">
                            {{ $detailsOrder->humanReadableDateTime($detailsOrder->booking_date) }}</p>
                    </div>
                    ...
                    <div class="bg-zinc-800 rounded-lg p-4 border border-zinc-700">
                        ...
                        <p class="text-zinc-100 font-medium">{{ $detailsOrder->pickupLocation?->name ?? 'N/A' }}
                        </p>
                    </div>
                ...
                {{-- Financial Summary --}}
                <div class="mt-6">
                    <h3 class="text-lg font-semibold text-zinc-400 mb-4">Financial Summary</h3>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div class="bg-zinc-800 rounded-lg p-4 border border-zinc-700">
                            <p class="text-xs text-zinc-400 uppercase tracking-wider mb-1">Subtotal</p>
                            <p class="text-zinc-100 font-semibold text-xl">
                                ${{ number_format($detailsOrder->subtotal ?? 0, 2) }}</p>
                        </div>

                        <div class="bg-zinc-800 rounded-lg p-4 border border-zinc-700">
                            <p class="text-xs text-zinc-400 uppercase tracking-wider mb-1">Delivery Fee</p>
                            <p class="text-zinc-100 font-semibold text-xl">
                                ${{ number_format($detailsOrder->delivery_fee ?? 0, 2) }}</p>
                        </div>

                        <div class="bg-zinc-800 rounded-lg p-4 border border-zinc-700">
                            <p class="text-xs text-zinc-400 uppercase tracking-wider mb-1">Service Fee</p>
                            <p class="text-zinc-100 font-semibold text-xl">
                                ${{ number_format($detailsOrder->service_fee ?? 0, 2) }}</p>
                        </div>

                        <div class="bg-zinc-800 rounded-lg p-4 border border-zinc-700">
                            <p class="text-xs text-zinc-400 uppercase tracking-wider mb-1">Tax Amount</p>
                            <p class="text-zinc-100 font-semibold text-xl">
                                ${{ number_format($detailsOrder->tax_amount ?? 0, 2) }}</p>
                        </div>

                        <div class="bg-zinc-800 rounded-lg p-4 border border-zinc-700">
                            <p class="text-xs text-zinc-400 uppercase tracking-wider mb-1">Security Deposit</p>
                            <p class="text-zinc-100 font-semibold text-xl">
                                ${{ number_format($detailsOrder->security_deposit ?? 0, 2) }}</p>
                        </div>

                        <div class="bg-emerald-800 rounded-lg p-4 border border-emerald-700">
                            <p class="text-xs text-emerald-100 uppercase tracking-wider mb-1">Total Amount</p>
                            <p class="text-emerald-100 font-bold text-2xl">
                                ${{ number_format($detailsOrder->total_amount ?? 0, 2) }}</p>
                        </div>
                ...
                @if ($detailsOrder->special_requests)
                    <div class="mt-6">
                        <h3 class="text-lg font-semibold text-zinc-400 mb-4">Special Requests</h3>
                        <div class="bg-zinc-800 rounded-lg p-4 border border-zinc-400">
                ...
                {{-- Audit Information --}}
                <div class="mt-6">
                    <h3 class="text-lg font-semibold text-zinc-400 mb-4">Audit Information</h3>
                        ...
                        @if ($detailsOrder->updatedBy)
                            <div class="bg-zinc-800 rounded-lg p-4 border border-zinc-700">
                                ...
                                <p class="text-xs text-zinc-400 mt-1">
                                    {{ $detailsOrder->updated_at->format('M d, Y H:i A') }}</p>
                            </div>
                        @endif
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    @if ($detailsOrder->residentialAddress)
                        ...
                        <div class="bg-zinc-800 rounded-lg p-6 border border-zinc-700">
                            ...
                            <div class="space-y-3">
                                <div>
                                    <p class="text-xs text-zinc-400 uppercase tracking-wider mb-1">Street Address
                                    </p>
                        ...
                        <div class="bg-zinc-800 rounded-lg p-6 border border-zinc-700">
                            <h4 class="text-lg font-semibold text-zinc-100 mb-4 flex items-center">
                                ...
                                @if ($detailsOrder->relation?->same_as_residential)
                                    <span
                                        class="ml-2 text-xs bg-emerald-500/20 text-emerald-400 px-2 py-1 rounded">Same
                                        as Residential</span>
                                @endif
                            </h4>
                            <div class="space-y-3">
                            ...
                            <div class="flex items-center justify-between">
                                <div>
                                    <p class="text-xs text-zinc-400 uppercase tracking-wider mb-1">Verification
                                        Status</p>
                                    <span
                                        class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium badge badge-soft {{ $doc->verification_status_color }}">
                                        {{ $doc->verification_status_label }}
                                    </span>
                                </div>
                                @if ($doc->verified_at)
                                    <div class="text-right">
                                        <p class="text-xs text-zinc-400 uppercase tracking-wider mb-1">Verified At
                                        </p>
                                        <p class="text-zinc-100 text-sm">
                                            {{ \Carbon\Carbon::parse($doc->verified_at)->format('M d, Y H:i A') }}
                                        </p>
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                @if ($doc->issue_date)
                                    <div class="bg-zinc-800/50 rounded-lg p-4 border border-zinc-700">
                                        <p class="text-xs text-zinc-400 uppercase tracking-wider mb-1">License
                                            Issue Date</p>
                                        <p class="text-zinc-100 font-medium">
                                            {{ \Carbon\Carbon::parse($doc->issue_date)->format('M d, Y') }}</p>
                                    </div>
                                @endif
                                ...
                                @if ($doc->expiry_date)
                                    <div class="bg-zinc-800/50 rounded-lg p-4 border border-zinc-700">
                                        <p class="text-xs text-zinc-400 uppercase tracking-wider mb-1">License
                                            Expiry Date</p>
                                        <p class="text-zinc-100 font-medium">
                                            {{ \Carbon\Carbon::parse($doc->expiry_date)->format('M d, Y') }}</p>
                                    </div>
                                @endif
                            </div>
                        @endif
                            ...
                            @if ($doc->licence)
                                <div class="bg-zinc-800 rounded-lg p-4 border border-zinc-700">
                                    ...
                                    <div class="aspect-video bg-zinc-900 rounded-lg overflow-hidden mb-3">
                                        <img src="{{ Storage::url($doc->licence) }}" alt="License"
                                    ...
                                    <a href="{{ Storage::url($doc->licence) }}" target="_blank"
                                        ...
                                        <svg class="w-4 h-4 mr-1" fill="none" stroke="white" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            @if ($doc->selfe_licence)
                                <div class="bg-zinc-800 rounded-lg p-4 border border-zinc-700">
                                    <p class="text-xs text-zinc-400 uppercase tracking-wider mb-3">Selfie with
                                        License</p>
                                    <div class="aspect-video bg-zinc-900 rounded-lg overflow-hidden mb-3">
                                        <img src="{{ Storage::url($doc->selfe_licence) }}" alt="Selfie"
                                            class="w-full h-full object-contain">
                                    </div>
                                    ...
                                    <div class="aspect-video bg-zinc-900 rounded-lg overflow-hidden mb-3">
                                        @if (Str::endsWith($doc->address_proof, '.pdf'))
                                            <div class="flex items-center justify-center h-full">
                                                <svg class="w-16 h-16 text-zinc-600" fill="currentColor"
                                                    viewBox="0 0 20 20">
                                        ...
                                        @else
                                            <img src="{{ Storage::url($doc->address_proof) }}" alt="Address Proof"
                                                class="w-full h-full object-contain">
                                        @endif
                                    </div>
                        ...
                        @if ($detailsOrder->relation?->signature_path)
                            <div class="bg-zinc-800 rounded-lg p-4 border border-zinc-700">
                                ...
                                <div class="bg-white rounded-lg p-4 max-w-md">
                                    <img src="{{ Storage::url($detailsOrder->relation->signature_path) }}"
                                ...
                                <div class="mt-3 flex items-center text-sm text-zinc-400">
                                    ...
                                    Terms accepted on
                                    {{ \Carbon\Carbon::parse($detailsOrder->relation->terms_accepted_at)->format('M d, Y H:i A') }}
                                </div>
                            </div>
                        @endif
                                ...
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-zinc-400">
                                            {{ $detailsOrder->relation->sms_alerts ? 'Enabled' : 'Disabled' }}</p>
                                    </div>
                                </div>
                            </div>
                        @endif
            ...
            <div x-show="activeTab === 'vehicle'" x-transition:enter="transition ease-out duration-200"
                ...
                @if ($detailsOrder->vehicle)
                    ...
                    <div class="space-y-6">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                @foreach ($vehicle->images->take(4) as $image)
                                    <div class="aspect-video bg-zinc-900 rounded-lg overflow-hidden">
                                        <img src="{{ storage_url($image->image_path) }}" alt="Vehicle"
                        ...
                        {{-- Pricing Info --}}
                        <div>
                            <h4 class="text-lg font-semibold text-zinc-400 mb-4">Pricing Information</h4>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="bg-zinc-800 rounded-lg p-4 border border-zinc-700">
                                    <p class="text-xs text-zinc-400 uppercase tracking-wider mb-1">Weekly Rate</p>
                                    <p class="text-zinc-100 font-semibold text-xl">
                                        ${{ number_format($vehicle->weekly_rate ?? 0, 2) }}</p>
                                </div>

                                <div class="bg-zinc-800 rounded-lg p-4 border border-zinc-700">
                                    <p class="text-xs text-zinc-400 uppercase tracking-wider mb-1">Monthly Rate</p>
                                    <p class="text-zinc-100 font-semibold text-xl">
                                        ${{ number_format($vehicle->monthly_rate ?? 0, 2) }}</p>
                                </div>

                                <div class="bg-zinc-800 rounded-lg p-4 border border-zinc-700">
                                    <p class="text-xs text-zinc-400 uppercase tracking-wider mb-1">Weekly Security
                                        Deposit</p>
                                    <p class="text-zinc-100 font-semibold text-xl">
                                        ${{ number_format($vehicle->security_deposit_weekly ?? 0, 2) }}</p>
                                </div>

                                <div class="bg-zinc-800 rounded-lg p-4 border border-zinc-700">
                                    <p class="text-xs text-zinc-400 uppercase tracking-wider mb-1">Monthly Security
                                        Deposit</p>
                                    <p class="text-zinc-100 font-semibold text-xl">
                                        ${{ number_format($vehicle->security_deposit_monthly ?? 0, 2) }}</p>
                                </div>
                            </div>
                        </div>
                            ...
                            @foreach ($detailsOrder->timeline->sortByDesc('created_at') as $timeline)
                                <div class="relative flex items-start">
                                    ...
                                    <div class="absolute left-8 -translate-x-1/2 mt-1.5">
                                        <div
                                            class="w-4 h-4 rounded-full border-4 border-zinc-900 
                                            ...
                                            <div class="flex items-start justify-between mb-2">
                                                <div>
                                                    <h5 class="text-zinc-100 font-semibold">
                                                        Status Changed to:
                                                        <span
                                                            class="capitalize">{{ $timeline->booking_status_label }}</span>
                                                    </h5>
                                                    <p class="text-xs text-zinc-400 mt-1">
                                                        {{ \Carbon\Carbon::parse($timeline->created_at)->format('M d, Y H:i A') }}
                                                        <span class="mx-1 text-zinc-100">•</span>
                                                        {{ \Carbon\Carbon::parse($timeline->created_at)->diffForHumans() }}
                                                    </p>
                                                </div>
                                                <span
                                                    class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium
                                                    {{ $timeline->booking_status === \App\Models\Booking::BOOKING_STATUS_ACCEPTED ? 'bg-emerald-500/100 text-emerald-400 border border-emerald-500/30' : '' }}
                                                    {{ $timeline->booking_status === \App\Models\Booking::BOOKING_STATUS_PENDING ? 'bg-amber-500/100 text-amber-400 border border-amber-500/30' : '' }}
                                                    {{ $timeline->booking_status === \App\Models\Booking::BOOKING_STATUS_REJECTED ? 'bg-red-500/100 text-red-400 border border-red-500/30' : '' }}
                                                    {{ $timeline->booking_status === \App\Models\Booking::BOOKING_STATUS_CANCELLED ? 'bg-red-500/100 text-red-400 border border-red-500/30' : '' }}
                                                    {{ $timeline->booking_status === \App\Models\Booking::BOOKING_STATUS_DEPOSITED ? 'bg-blue-500/100 text-blue-400 border border-blue-500/30' : '' }}
                                                    {{ $timeline->booking_status === \App\Models\Booking::BOOKING_STATUS_DELIVERED ? 'bg-purple-500/100 text-purple-400 border border-purple-500/30' : '' }}
                                                    {{ $timeline->booking_status === \App\Models\Booking::BOOKING_STATUS_RETURNED ? 'bg-green-500/100 text-green-400 border border-green-500/30' : '' }}">
                                                    {{ $timeline->booking_status_label }}
                                                </span>
                                            </div>

                                            @if ($timeline->createdBy)
                                                <div class="flex items-center mt-3 pt-3 border-t border-zinc-100">
                                                    <svg class="w-4 h-4 text-zinc-100 mr-2" fill="none"
                                                    ...
                                                    <span
                                                        class="text-sm text-zinc-300">{{ $timeline->createdBy->name }}</span>
                                                    <span class="mx-2 text-zinc-100">•</span>
                                                    <span
                                                        class="text-xs text-zinc-400">{{ $timeline->createdBy->email }}</span>
                                                </div>
                                            @endif
                                        </div>
                                    </div>
                    ...
                    <div class="px-6 py-4 space-y-4">
                        <div>
                            ...
                            <textarea wire:model="reason"
                                ...
                                rows="5" placeholder="Enter the reason for rejection (minimum 10 characters)" required></textarea>
                            @error('reason')

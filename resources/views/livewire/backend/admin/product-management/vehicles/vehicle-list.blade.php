            ...
            <div>
                <table class="w-full">
                    <thead class="bg-zinc-600/50 border-b border-zinc-700">
                        <tr>
                            <th class="px-6 text-white py-4 text-left text-xs font-semibold uppercase tracking-wider">SL
                            </th>
                            ...
                            <tr class="bg-zinc-50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-col">
                                        <span class="text-accent text-sm">{{ $vehicle->created_at_formatted }}</span>
                                        <span class="text-zinc-500 text-xs">{{ $vehicle->created_at_human }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-accent">
                                    {{ $vehicle->createdBy?->name ?? 'System' }}
                                </td>
                                <td class="px-6 py-4">
                                    <div x-data="{ open: false }" class="relative flex justify-end">
                                        <!-- Dropdown Trigger -->
                                        <button @click="open = !open"
                                            class="p-2 text-gray-500 hover:text-gray-700 rounded-lg transition-all duration-200"
                                            title="Actions">
                                            <svg xmlns="http://www.w3.org/2000/svg"
                                        ...
                                        <div x-show="open" @click.away="open = false"
                                            ...
                                            class="absolute right-4 top-4 mt-2 w-40 bg-white border border-gray-100 rounded-xl shadow-lg z-50">

                                            <a href="{{ route('admin.pm.vehicle-details', $vehicle->id) }}"
                                                wire:navigate
                                                class="block px-4 py-2 text-sm text-blue-600 hover:bg-blue-50 rounded-t-xl flex items-center gap-2">
                                                <flux:icon name="eye" class="w-4 h-4" />
                                                Details
                                            </a>

                                            <a href="{{ route('admin.pm.vehicle-edit', $vehicle->id) }}" wire:navigate
                                                class="block px-4 py-2 text-sm text-amber-600 hover:bg-amber-50 flex items-center gap-2">
                            ...
                            <tr>
                                <td colspan="8" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center justify-center gap-2">
                                        <flux:icon name="info" class="w-12 h-12 text-zinc-600" />
                                        <p class="text-zinc-500 text-lg">{{ __('No vehicles found') }}</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse

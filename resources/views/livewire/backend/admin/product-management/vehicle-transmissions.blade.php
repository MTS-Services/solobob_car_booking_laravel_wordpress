<div>
  <div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full overflow-y-auto!">
                    ...
                    <tbody class="divide-y divide-zinc-700/50">
                        @forelse ($vehicleTransmissions as $vehicleTransmission)
                            <tr class="bg-zinc-50 transition-colors duration-150">
                               
                                <td class="px-6 py-4 text-accent">{{ $vehicleTransmission->name }}</td>
                                <td class="px-6 py-4 text-accent">{{ $vehicleTransmission->slug }}</td>
                                
                                <td class="px-6 py-4">
                                    <div class="flex flex-col">
                                        <span class="text-accent text-sm">{{ $vehicleTransmission->created_at_formatted }}</span>
                                        <span class="text-zinc-500 text-xs">{{ $vehicleTransmission->created_at_human }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-accent">
                                    {{ $vehicleTransmission->createdBy?->name ?? 'System' }}
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-end">
                                        <div class="relative" x-data="{ open: false }">
                                            ...
                                            <div x-show="open" x-transition:enter="transition ease-out duration-100"
                                                ...
                                                style="display: none;">
                                                <div class="py-1">
                                                    <button wire:click="openDetailsModal({{ $vehicleTransmission->id }})"
                                                        @click="open = false"
                                                    ...
                                                    <button wire:click="openEditModal({{ $vehicleTransmission->id }})"
                                                        @click="open = false"
                                                    ...
                                                    <button wire:click="openDeleteModal({{ $vehicleTransmission->id }})"
                                                        @click="open = false"
    ...
    {{-- Details Modal --}}
    @if ($showDetailsModal && $detailsVehicleTransmission)
                ...
                <div
                    ...
                    <div class="px-6 py-6 space-y-6">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <h4 class="text-xl font-semibold text-zinc-100">{{ $detailsVehicleTransmission->name }}</h4>
                                <p class="text-zinc-400">{{ $detailsVehicleTransmission->slug }}</p>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="bg-zinc-800/50 rounded-lg p-4 border border-zinc-700">
                                ...
                                <p class="text-zinc-200 font-medium">#{{ $detailsVehicleTransmission->id }}</p>
                            </div>

                          
                            <div class="bg-zinc-800/50 rounded-lg p-4 border border-zinc-700">
                                <p class="text-xs text-zinc-500 uppercase tracking-wider mb-1">Created At</p>
                                <p class="text-zinc-200 font-medium">{{ $detailsVehicleTransmission->created_at_formatted }}</p>
                                <p class="text-xs text-zinc-400 mt-1">{{ $detailsVehicleTransmission->created_at_human }}</p>
                            </div>

                            <div class="bg-zinc-800/50 rounded-lg p-4 border border-zinc-700">
                                <p class="text-xs text-zinc-500 uppercase tracking-wider mb-1">Created By</p>
                                <p class="text-zinc-200 font-medium">{{ $detailsVehicleTransmission->createdBy?->name ?? 'System' }}
                                </p>
                                @if ($detailsVehicleTransmission->createdBy)
                                    <p class="text-xs text-zinc-400 mt-1">{{ $detailsVehicleTransmission->createdBy->slug }}</p>
                                @endif
                            </div>

                            @if ($detailsVehicleTransmission->updated_at && $detailsVehicleTransmission->updated_at != $detailsVehicleTransmission->created_at)
                                <div class="bg-zinc-800/50 rounded-lg p-4 border border-zinc-700">
                                    <p class="text-xs text-zinc-500 uppercase tracking-wider mb-1">Updated At</p>
                                    <p class="text-zinc-200 font-medium">{{ $detailsVehicleTransmission->updated_at_formatted }}</p>
                                    <p class="text-xs text-zinc-400 mt-1">{{ $detailsVehicleTransmission->updated_at_human }}</p>
                                </div>

                                @if ($detailsVehicleTransmission->updatedBy)
                                    <div class="bg-zinc-800/50 rounded-lg p-4 border border-zinc-700">
                                        <p class="text-xs text-zinc-500 uppercase tracking-wider mb-1">Updated By</p>
                                        <p class="text-zinc-200 font-medium">{{ $detailsVehicleTransmission->updatedBy->name }}</p>
                                        <p class="text-xs text-zinc-400 mt-1">{{ $detailsVehicleTransmission->updatedBy->email }}
                                        </p>
                                    </div>
                                @endif
                            @endif

                            @if ($detailsVehicleTransmission->deleted_at)
                                <div class="bg-zinc-800/50 rounded-lg p-4 border border-zinc-700">
                                    <p class="text-xs text-zinc-500 uppercase tracking-wider mb-1">Deleted At</p>
                                    <p class="text-zinc-200 font-medium">{{ $detailsVehicleTransmission->deleted_at_formatted }}</p>
                                    <p class="text-xs text-zinc-400 mt-1">{{ $detailsVehicleTransmission->deleted_at_human }}</p>
                                </div>

                                @if ($detailsVehicleTransmission->deletedBy)
                                    <div class="bg-zinc-800/50 rounded-lg p-4 border border-zinc-700">
                                        <p class="text-xs text-zinc-500 uppercase tracking-wider mb-1">Deleted By</p>
                                        <p class="text-zinc-200 font-medium">{{ $detailsVehicleTransmission->deletedBy->name }}</p>
                                        <p class="text-xs text-zinc-400 mt-1">{{ $detailsVehicleTransmission->deletedBy->email }}
                                        </p>
                                    </div>
                                @endif
                            @endif
